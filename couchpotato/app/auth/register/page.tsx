	...
	const handleSubmit = async (data: FormData) => {
		...
		try {
			...
			if (response.ok) {
				...
				// Use a timeout to ensure the success message is seen before redirecting
				setTimeout(() => {
					// Use Next.js router or window.location for navigation after delay
					window.location.href = `/profile/${responseData.user.username}`;

				}, 1500);
			} else {
				// Display appropriate error message based on status code
				const errorMessage = responseData.error || `Registration failed (${response.status})`;
